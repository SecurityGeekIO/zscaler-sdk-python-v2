    ...
    def test_app_connector_schedule(self, fs):
        ...
        try:
            ...
            if "resource.already.exist" not in str(exc):
                errors.append(f"Error during add_connector_schedule: {exc}")
            else:
        ...
        try:
            ...
            assert updated_schedule is not None, "Failed to update App Connector Schedule"
            pprint(updated_schedule)

        except Exception as exc:
            errors.append(f"Error during update_connector_schedule: {exc}")
