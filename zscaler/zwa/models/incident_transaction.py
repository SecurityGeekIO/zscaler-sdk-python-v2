...
class DLPIncidentTransaction(ZscalerObject):
    """
    A class for DLPIncidentTransaction objects.
    """

    def __init__(self, config=None):
        """
        Initialize the DLPIncidentTransaction model based on API response.
        ...
        if config:
            ...
            self.incidents = ZscalerCollection.form_list(
                config["incidents"] if "incidents" in config else [], str
            )
        else:
            self.cursor = None
