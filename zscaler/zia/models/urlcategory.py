...
class URLCategory(ZscalerObject):
    ...
    def __init__(self, config=None):
        ...
        if config:
            ...
            self.configured_name = config["configuredName"] \
                if "configuredName" in config else None
            self.super_category = config["superCategory"] \
                if "superCategory" in config else None

            self.keywords = ZscalerCollection.form_list(
                config["keywords"] if "keywords" in config else [], str
            )

            self.keywords_retaining_parent_category = ZscalerCollection.form_list(
                config["keywordsRetainingParentCategory"] if "keywordsRetainingParentCategory"\
                    in config else [], str
            )
            ...
            self.ip_ranges = ZscalerCollection.form_list(
                config["ipRanges"] if "ipRanges" in config else [], str
            )

            self.ip_ranges_retaining_parent_category = ZscalerCollection.form_list(
                config["ipRangesRetainingParentCategory"] if "ipRangesRetainingParentCategory"\
                    in config else [], str
            )

            self.custom_category = config["customCategory"] \
                if "customCategory" in config else False
            ...
            if "scopes" in config:
                for scope in config["scopes"]:
                    scope_group = {
                        ...
                        "ScopeEntities": ZscalerCollection.form_list(
                            scope.get("ScopeEntities", []), dict
                        )
                    }
            ...
            self.url_keyword_counts = {
                "totalUrlCount": config["urlKeywordCounts"]["totalUrlCount"] \
                    if "urlKeywordCounts" in config and "totalUrlCount" in config["urlKeywordCounts"] else 0,
                "retainParentUrlCount": config["urlKeywordCounts"]["retainParentUrlCount"] \
                    if "urlKeywordCounts" in config and "retainParentUrlCount" in config["urlKeywordCounts"] else 0,
                "totalKeywordCount": config["urlKeywordCounts"]["totalKeywordCount"] \
                    if "urlKeywordCounts" in config and "totalKeywordCount" in config["urlKeywordCounts"] else 0,
                "retainParentKeywordCount": config["urlKeywordCounts"]["retainParentKeywordCount"] \
                    if "urlKeywordCounts" in config and "retainParentKeywordCount" in config["urlKeywordCounts"] else 0
            } if "urlKeywordCounts" in config else None

            self.custom_urls_count = config["customUrlsCount"] \
                if "customUrlsCount" in config else 0
            self.urls_retaining_parent_category_count = config["urlsRetainingParentCategoryCount"] \
                if "urlsRetainingParentCategoryCount" in config else 0
            self.custom_ip_ranges_count = config["customIpRangesCount"] \
                if "customIpRangesCount" in config else 0
            self.ip_ranges_retaining_parent_category_count = config["ipRangesRetainingParentCategoryCount"] \
                if "ipRangesRetainingParentCategoryCount" in config else 0
        else:
...
class UrlDomainReview(ZscalerObject):
    ...
    def __init__(self, config=None):
        ...
        if config:
            ...
            self.domain_type = config["domainType"] \
                if "domainType" in config else None

            # Handling matches list of objects
            self.matches = ZscalerCollection.form_list(
                config["matches"] if "matches" in config else [], dict
            )
        else:
            # Defaults when config is None
            self.url = None
