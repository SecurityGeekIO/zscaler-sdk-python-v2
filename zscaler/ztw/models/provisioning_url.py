    ...
    def __init__(self, config=None):
        ...
        if config:
            ...
            if "location" in config:
                if isinstance(config["location"], common.CommonIDNameExternalIDIDNameExternalID):
                    self.location = config["location"]
                elif config["location"] is not None:
                    self.location = common.CommonIDNameExternalIDIDNameExternalID(config["location"])
                else:
                    self.location = None
            else:
                self.location = None
            ...
            if "lastModUid" in config:
                if isinstance(config["lastModUid"], common.CommonIDNameExternalIDIDNameExternalID):
                    self.last_mod_uid = config["lastModUid"]
                elif config["lastModUid"] is not None:
                    self.last_mod_uid = common.CommonIDNameExternalIDIDNameExternalID(config["lastModUid"])
                else:
